...
export function BankingBin({ open, onClose, listMutation, ...other }: Props) {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(CONFIG.pageSizesGlobal);
    const typeRestore = EntityRestoreType;

    const { bankAccounts, pagination, bankAccountsLoading, bankAccountsEmpty, mutation } = useGetDeletedBankAccounts({
        pageNumber: page + 1,
    ...
    const handleRestore = async (ids: string[]) => {
        ...
        try {
            ...
            const res = await restoreAll(ids, typeRestore.BankAccounts);
            console.log(res);
