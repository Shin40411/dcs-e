...
export function UnitBin({ open, onClose, listMutation, ...other }: Props) {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(CONFIG.pageSizesGlobal);
    const typeRestore = EntityRestoreType;

    const { units, pagination, unitsLoading, unitsEmpty, mutation } = useGetDeletedUnits({
        pageNumber: page + 1,
    ...
    const handleRestore = async (ids: string[]) => {
        ...
        try {
            ...
            const res = await restoreAll(ids, typeRestore.Units);
            console.log(res);
