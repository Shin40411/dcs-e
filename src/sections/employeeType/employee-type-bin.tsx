...
export function EmployeeTypeBin({ open, onClose, listMutation, ...other }: Props) {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(CONFIG.pageSizesGlobal);
    const typeRestore = EntityRestoreType;

    const { employeeTypes, pagination, employeeTypesLoading, employeeTypesEmpty, mutation } = useGetDeletedEmployeeTypes({
        pageNumber: page + 1,
    ...
    const handleRestore = async (ids: string[]) => {
        ...
        try {
            ...
            const res = await restoreAll(ids, typeRestore.EmployeeTypes);
            console.log(res);
