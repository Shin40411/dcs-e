...
export function CustomerBin({ open, onClose, listMutation, ...other }: Props) {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(CONFIG.pageSizesGlobal);
    const typeRestore = EntityRestoreType;

    const { customers, pagination, customersLoading, customersEmpty, mutation } = useGetDeletedCustomers({
        pageNumber: page + 1,
    ...
    const handleRestore = async (ids: string[]) => {
        ...
        try {
            ...
            const res = await restoreAll(ids, typeRestore.Customers);
            console.log(res);
