...
import { IReceiverInternal } from "src/types/internal";
...
export function SpendFilterBar({ onFilterChange, onSearching, onReset }: Props) {
    const today = dayjs();
    ...
    const [selectedReceiver, setSelectedReceiver] = useState<IReceiverInternal | null>(null);

    const { result, isLoading } = useGetPayerOrReceiver<IReceiverInternal>({
        pageNumber: 1,
        pageSize: 20,
        getPayer: false,
        enabled: true,
    ...
    const handleApply = () => {
        onFilterChange({
            ...
            receiverName: selectedReceiver?.receiverName || undefined,
        ...
        >
            ...
            >
                <Autocomplete
                    ...
                    getOptionLabel={(option) => option.receiverName || "Chưa cập nhật"}
                    onChange={(_, newValue) => setSelectedReceiver(newValue)}
                    renderInput={(params) => (
                        <TextField
                            {...params}
