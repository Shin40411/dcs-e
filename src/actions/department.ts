...
export function useGetDepartments({ pageNumber, pageSize, key, enabled = true }: departmentProps) {
    ...
    const { data, isLoading, error, isValidating, mutate } = useSWR<ResDepartmentList>(url, fetcher, swrOptions);
...
export function useGetDeletedDepartments({ pageNumber, pageSize, key, enabled = true }: departmentProps) {
    let params = '';
